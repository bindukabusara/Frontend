  ...
  const fetchReminders = async () => {
    try {
      ...
      const [remindersResponse, ordersResponse] = await Promise.all([
        axios.get("http://backend-zltr.onrender.com/api/reminders", { headers }).catch(() => ({ data: [] })),
        axios.get("http://backend-zltr.onrender.com/api/orders/confirmed", { headers }).catch(() => ({ data: [] }))
      ]);
    ...
    try {
      ...
      if (isFromOrder) {
        response = await axios.post(
          `http://backend-zltr.onrender.com/api/orders/provide-instructions`,
          { orderId: reminderId, status: "completed" },
          { headers }
        );
      } else {
        response = await axios.post(
          `http://backend-zltr.onrender.com/api/reminders/${reminderId}/complete`,
          {},
          { headers }
        );
      }
                    ...
                    <tr key={reminder._id}>
                      <td>
                        <div className="d-flex align-items-center">
                          <div className="me-3">
                            <img
                              src={`http://backend-zltr.onrender.com/uploads/${reminder.medicationImage || 'default-medication.png'}`}
          ...
          {selectedReminder && (
            <div className="reminder-details">
              <div className="d-flex mb-4">
                <div className="me-4">
                  <img
                    src={`http://backend-zltr.onrender.com/uploads/${selectedReminder.medicationImage || 'default-medication.png'}`}
